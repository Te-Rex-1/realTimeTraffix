...
cap = cv2.VideoCapture('Data/Video/higway_mini.mp4')
...
while True:
    ...
    for bbox in bbox_id:
        ...
        if id in down:
            if blue_line_y < (cy + offset) and blue_line_y > (cy - offset):
                ...
                # counter+=1
                counter_down.append(
                    id)  # get a list of the cars and buses which are entering the line red and exiting the line blue
    ...
    cv2.putText(frame, ('going down - ') + str(downwards), (60, 40), cv2.FONT_HERSHEY_SIMPLEX, 0.5, green_color, 1,
                cv2.LINE_AA)

    upwards = (len(counter_up))
    cv2.putText(frame, ('going up - ') + str(upwards), (60, 60), cv2.FONT_HERSHEY_SIMPLEX, 0.5, text_color, 1,
                cv2.LINE_AA)
